...
from 05_list_index_getter import get_item_at
...
def test_seq_must_be_list_or_tuple(seq):
    with pytest.raises(TypeError) as ei:
        get_item_at(seq, 0)
    assert str(ei.value) == "seq must be a list or tuple", f"expected {'seq must be a list or tuple'!r} but got {str(ei.value)!r}"
...
def test_index_must_be_int(index):
    ...
    assert str(ei.value) == "index must be an int", f"expected {'index must be an int'!r} but got {str(ei.value)!r}"
...
@pytest.mark.parametrize("seq,index", [([1], 1), ([1], 2), ([], 0), (("a",), 5), (("a", "b"), -3)])
def test_index_out_of_range_raises_indexerror(seq, index):
    with pytest.raises(IndexError) as ei:
        get_item_at(seq, index)
    assert str(ei.value) == "index out of range", f"expected {'index out of range'!r} but got {str(ei.value)!r}"
