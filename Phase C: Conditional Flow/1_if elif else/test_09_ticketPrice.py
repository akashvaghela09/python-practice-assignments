...
def _run_script(path: Path, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("mod_09_ticketPrice", str(path))
...
def test_ticket_price(monkeypatch):
    path = Path(__file__).resolve().parent / "09_ticketPrice.py"
    actual = _run_script(path, monkeypatch)
    expected = "Ticket price: 7\n"
    if actual != expected:
