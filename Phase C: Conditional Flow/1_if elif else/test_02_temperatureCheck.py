...
def _run_script(path: Path, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("mod_02_temperatureCheck", str(path))
...
def test_temperature_label(monkeypatch):
    path = Path(__file__).resolve().parent / "02_temperatureCheck.py"
