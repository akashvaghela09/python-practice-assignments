...
def _run_script(path: Path, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("mod_06_shippingCost", str(path))
...
def test_shipping_cost(monkeypatch):
    path = Path(__file__).resolve().parent / "06_shippingCost.py"
    actual = _run_script(path, monkeypatch)
    expected = "Shipping cost: 8\n"
    if actual != expected:
